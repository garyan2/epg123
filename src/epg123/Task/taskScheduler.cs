    ...
    internal class epgTaskScheduler
    {
        ...
        public bool CreateTask(bool wakeToRun, string startTime, TaskActions[] tActions)
        {
            ...
            else
            {
                ...
                Logger.WriteInformation("Successfully wrote task xml file for import when running with elevated rights.");
            }
            return true;
        }
        ...
        {
            ITaskDefinition newTask = taskService.NewTask(0);
            newTask = taskService.GetFolder("\\Microsoft\\Windows\\Media Center").GetTask("mcupdate").Definition;
